...
The real and imaginary part allow to keep track for the 2 dimensional
directions.
...
data_dictionary = {complex(i,j): c for j, r in enumerate(open('data.txt'))
...
def count_energized_tiles(tiles_to_visit: list) -> int:
    ...
    and characters as values (representing tile types). The function simulates the movement of a beam
    through the grid, starting from a given position and direction. The beam's direction changes based
    on the type of tile it encounters ('|', '-', '/', '\\').
    

    Args:
        tiles_to_visit (list): A list of tuples, each containing a starting position (complex number)
    ...
    Note:
        The function assumes that the grid is represented in 'data_dictionary' and has been populated
        from a file named 'data.txt'. The file should contain a grid representation with specific characters.
    '''
...
RESULT = count_energized_tiles(tiles_with_beam)
print(RESULT)