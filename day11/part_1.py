...
def increment_greater_than_key(lst, key):
    """This function adds 1 to all list elements greater than key."""
    return [x + 1 if x > key else x for x in lst]
...
for value in sorted(missing_x,reverse=True): # Sorting is necessary.
    x_coords = increment_greater_than_key(x_coords, value)
for value in sorted(missing_y,reverse=True):
    y_coords = increment_greater_than_key(y_coords, value)
